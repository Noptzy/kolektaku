...
async function resolveViaDoH(hostname) {
  ...
  try {
    ...
    const answer = dohRes.data?.Answer?.find((r) => r.type === 1);
    if (answer?.data) {
...
async function resolveEpisode(url, workerId) {
  ...
  try {
    ...
    logger.info(
      `[DEBUG] servers response status: ${serversRes.status}, data keys: ${Object.keys(serversRes.data || {}).join(", ")}, html length: ${html?.length || 0}`,
      { worker: workerId },
    );

    if (!html)
      throw new Error(
        `Gagal mendapatkan HTML servers. Status: ${serversRes.data?.status}, Response: ${JSON.stringify(serversRes.data).substring(0, 200)}`,
      );
    ...
    logger.info(`Extracted iframe link: ${link}`, { worker: workerId });
    ...
    logger.info(`Fetching getSources for rapidCloudId: ${rapidCloudId}`, {
      worker: workerId,
    });
